...
var halo=function(){
	...
	var use=function(){//载入用户自己组件方法
		var arg=arguments,need_load_count=0,loaded_count=0,cb=function(){},wait=needWait,names='',rechkModules={};
					...
					combo?(names+=','+path+name+'.js'):(function(name){
						load_module(name,function(ret){
							if('success'){//console.log(name)
								!!modules[name]?++loaded_count:(rechkModules[name]=1);//有可能是组件里使用了use，所以以modules[name]为准 
							}else{
		...
		function chkLoad(){
			//检查需要rechk的modules
			for(var i in rechkModules){
				!!modules[i]&&(++loaded_count,delete rechkModules[i]);
			}
			//检查加载情况
